...
function Productdetail() {
  ...
  const images = product.img && product.img.length > 0 ? product.img : ['https://via.placeholder.com/600'];
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-wrap items-center justify-between gap-4">
          ...
          <nav className="text-xs text-slate-400 flex items-center gap-2">
            ...
            <span>/</span>
            <span className="hover:text-yellow-400 cursor-pointer">{product.category || 'Electronics'}</span>
            <span>/</span>
            <span className="hover:text-yellow-400 cursor-pointer">{product.brand || 'Gadgets'}</span>
            <span>/</span>
            <span className="text-white font-medium truncate max-w-[180px] sm:max-w-[300px]">{product.name}</span>
          </nav>
        </div>
          ...
          <div className="lg:col-span-5 flex flex-col justify-between space-y-5">
            <div>
              ...
              <div className="mt-4 space-y-2 text-xs text-slate-400 border-t border-b border-slate-800/80 py-3">
                <div className="grid grid-cols-3">
                  <span>Made in:</span>
                  <span className="col-span-2 text-slate-200 font-medium">Australia</span>
                </div>
                <div className="grid grid-cols-3">
                  <span>Design:</span>
                  <span className="col-span-2 text-slate-200 font-medium">Modern</span>
                </div>
                <div className="grid grid-cols-3">
                  <span>Delivery:</span>
                  <span className="col-span-2 text-slate-200 font-medium">2 days delivery</span>
                </div>
              </div>
              ...
              <div className="mt-4 space-y-4">
                {/* Variant Options */}
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-2">Options</label>
                ...
                {/* Quantity */}
                <div>
                  <label className="text-xs font-semibold text-slate-300 block mb-2">Quantity</label>
              ...
              {/* Price */}
              <div className="mt-5">
                <span className="text-xs text-slate-400 block uppercase tracking-wider">Price</span>
                <span className="text-3xl font-black text-yellow-400">
                  ${(product.price ? product.price * quantity : 298.00).toFixed(2)}
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              ...
              <button className="p-3 border border-slate-800 bg-slate-900 rounded-xl hover:border-slate-700 text-slate-400 hover:text-red-400 transition">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-1.137-7.303a4.5 4.5 0 00-5.242 1.08L12 6.5l-.837-.837a4.5 4.5 0 00-6.364 0z" />
                </svg>
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          ...
          <div className="lg:col-span-8 bg-slate-900/60 rounded-2xl border border-slate-800 p-6 shadow-xl space-y-6">
            ...
            <div className="border-b border-slate-800 flex gap-8">
              {['Description', 'Reviews', 'Company', 'Usage guide'].map((tab) => (
                <button
                  key={tab}
                ...
                <div className="pt-4 space-y-2 border-t border-slate-800">
                  {product.specification ? (
                    Object.entries(product.specification).map(([key, val]) => (
                      <div key={key} className="grid grid-cols-3 py-2 border-b border-slate-800/50">
                    ...
                    <>
                      <div className="grid grid-cols-3 py-2 border-b border-slate-800/50">
                        ...
                        <span className="col-span-2 text-white font-medium">128 GB</span>
                      </div>
                      <div className="grid grid-cols-3 py-2 border-b border-slate-800/50">
                        <span className="text-slate-400">Material type</span>
                        <span className="col-span-2 text-white font-medium">Plastic and Metallic</span>
                      </div>
                    </>
                  )}
                </div>
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <div className="bg-slate-900/60 rounded-2xl border border-slate-800 p-5 shadow-xl space-y-4">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h4 className="text-xs font-bold text-white">Guanjoi Trading LLC</h4>
                  <div className="flex items-center gap-1 text-[11px] text-slate-400 mt-0.5">
                    <span className="text-amber-400">★</span> 4.2 • 1290 items sold
                  </div>
                </div>
              </div>
            ...
            <div className="bg-slate-900/60 rounded-2xl border border-slate-800 p-5 shadow-xl space-y-4">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-bold text-white">4.7 out of 5</span>
              </div>
              ...
              <div className="space-y-2 text-[11px]">
                {[
                  ...
                  <div key={row.star} className="flex items-center gap-2 text-slate-400">
        ...
        {relateproduct.length > 0 && (
          <div className="space-y-4 pt-4 border-t border-slate-800">
            <h3 className="text-lg font-bold text-white">
              Related Products from <span className="text-yellow-400">{product.brand}</span>
            </h3>

            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
              {relateproduct.map((item) => {
                const itemImg = Array.isArray(item.img) ? item.img[0] : item.img;
